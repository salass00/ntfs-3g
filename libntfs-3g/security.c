...
static int entersecurity_stuff(ntfs_volume *vol, fbx_off_t offs)
{
	int res;
	int written;
	unsigned long total;
...
static int entersecurity_data(ntfs_volume *vol,
			...
			le32 hash, le32 keyid, fbx_off_t offs, int gap)
{
	int res;
	int written1;
	int written2;
...
static int entersecurity_indexes(ntfs_volume *vol, s64 attrsz,
			le32 hash, le32 keyid, fbx_off_t offs)
{
	union {
		struct {
			le32 dataoffsl;
...
static BOOL groupmember(struct SECURITY_CONTEXT *scx, uid_t uid, gid_t gid)
{
	...
	if (scx->vol->secure_flags & (1 << SECURITY_STATICGRPS))
		...
		tid = scx->tid;
		snprintf(filename,sizeof(filename),"/proc/%u/cred",tid);
...
int ntfs_get_owner_mode(struct SECURITY_CONTEXT *scx,
		ntfs_inode * ni, struct fbx_stat *stbuf)
{
	const SECURITY_DESCRIPTOR_RELATIVE *phead;
	char *securattr;
	const SID *usid;	/* owner of file/directory */
...
static int localread(void *fileid, char *buf, size_t size, fbx_off_t offs)
{
