...
int ntfs_attr_data_read(ntfs_inode *ni,
		...
		char *buf, size_t size, fbx_off_t offset)
{
	ntfs_attr *na = NULL;
	int res, total = 0;
	...
	if ((size_t)offset < (size_t)na->data_size) {
		...
		while (size) {
			...
			if ((fbx_off_t)res < (fbx_off_t)size)
...
int ntfs_attr_data_write(ntfs_inode *ni, ntfschar *stream_name,
		int stream_name_len, const char *buf, size_t size, fbx_off_t offset)
{
	ntfs_attr *na = NULL;
	int res, total = 0;
...
int ntfs_attr_shrink_size(ntfs_inode *ni, ntfschar *stream_name,
		int stream_name_len, fbx_off_t offset)
{
	ntfs_attr_search_ctx *ctx;
	ATTR_RECORD *a;
	int res;
