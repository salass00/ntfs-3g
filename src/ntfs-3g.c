...
static int ntfs_fuse_readlink(const char *org_path, char *buf, size_t buf_size)
{
	...
	if (ni->flags & FILE_ATTR_REPARSE_POINT) {
		...
		target = ntfs_make_symlink(ni, ctx->abs_mnt_point, &attr_size);
		if (target) {
			strlcpy(buf,target,buf_size);
			free(target);
		} else
			if (errno == EOPNOTSUPP)
				strlcpy(buf,ntfs_bad_reparse,buf_size);
			else
				res = -errno;
		goto exit;
	}
		...
		const unsigned dt_type __attribute__((unused)))
{
	...
	if (ntfs_fuse_is_named_data_stream(filename)) {
		...
		struct fbx_stat st = { .st_ino = MREF(mref) };
...
static int ntfs_fuse_readdir(const char *path, void *buf,
		fuse_fill_dir_t filler, fbx_off_t offset __attribute__((unused)),
...
static int ntfs_fuse_read(const char *org_path, char *buf, size_t size,
		fbx_off_t offset, struct fuse_file_info *fi __attribute__((unused)))
{
	ntfs_inode *ni = NULL;
	ntfs_attr *na = NULL;
	char *path = NULL;
...
#endif /* HAVE_SETXATTR */
	if (offset + (fbx_off_t)size > max_read) {
		if (max_read < offset)
			goto ok;
...
static int ntfs_fuse_trunc(const char *org_path, fbx_off_t size,
#if !KERNELPERMS | (POSIXACLS & !KERNELACLS)
			BOOL chkwrite)
#else
...
static int ntfs_fuse_truncate(const char *org_path, fbx_off_t size)
{
	return ntfs_fuse_trunc(org_path, size, TRUE);
}

static int ntfs_fuse_ftruncate(const char *org_path, fbx_off_t size,
			struct fuse_file_info *fi __attribute__((unused)))
{
	/*
...
static int ntfs_fuse_utimens(const char *path, const struct timespec tv[2])
{
	...
	if (ntfs_allowed_as_owner(&security, ni)) {
		...
		ni->last_access_time = timespec2ntfs(tv[0]);
		ni->last_data_change_time = timespec2ntfs(tv[1]);
...
static void * ntfs_init_amigaos(struct fuse_conn_info *conn);
#endif
...
static void setup_logging(char *parsed_options)
{
	...
	ntfs_log_info("Mounted %s (%s, label \"%s\", NTFS %d.%d)\n",
			opts.device, ((ctx->ro) || NVolReadOnly(ctx->vol)) ? "Read-Only" : "Read-Write",
...
static void * ntfs_init_amigaos(struct fuse_conn_info *conn) {
	...
	char *parsed_options = fuse_get_context()->private_data;
...
#endif
	/* We must do ctx->blkdev his after ntfs_open() to be able to set the blksize */
	if (ctx->blkdev && set_fuseblk_options(&parsed_options))
	   goto err_out;
	...
	if (ctx->vol->vol_name && ctx->vol->vol_name[0]) {
		strlcpy(conn->volume_name, ctx->vol->vol_name, CONN_VOLUME_NAME_BYTES);
	} else {
		// make something up
		snprintf(conn->volume_name, CONN_VOLUME_NAME_BYTES, "NTFS-%s", opts.device);
	}
...
err_out:
	...
	return err == NTFS_VOLUME_NO_DISK ? NULL : (void *)-1;
}

int ntfs3g_main(struct Message *msg)
{
	char *parsed_options = NULL;
	int err;
	...
	// lets query some info from handler framework
	opts.device = FbxQueryMountMsg(msg, FBXQMM_MOUNT_NAME);
	opts.options = FbxQueryMountMsg(msg, FBXQMM_MOUNT_CONTROL);
	if (!opts.options) opts.options = strdup("");
	opts.mnt_point = ntfs_malloc(strlen(opts.device) + 2);
	if (!opts.mnt_point) {
	   err = NTFS_VOLUME_OUT_OF_MEMORY;
	   goto err_out;
	}
	strcpy(opts.mnt_point, "/");
	strcat(opts.mnt_point, opts.device);

	// parse options relating to ntfs filesystem
	parsed_options = parse_mount_options(ctx, &opts, FALSE);
	if (!parsed_options) {
	   err = NTFS_VOLUME_SYNTAX_ERROR;
	   goto err_out;
	...
	struct TagItem fs_tags[] = {
		...
		{ FBXT_DOSTYPE,     ID_NTFS_DISK                                             },
		{ FBXT_GET_CONTEXT, (IPTR)&_fuse_context_                                    },
		{ TAG_END,          0                                                        }
	};

	// the fuse_new() equivalent
	fs = FbxSetupFS(msg, fs_tags, &ntfs_3g_ops, sizeof(ntfs_3g_ops), parsed_options);
