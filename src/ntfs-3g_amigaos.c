...
#include "runlist.h"
#include "ntfstime.h"
#include "ntfsprogs/utils.h"
#include "ntfsprogs/sd.h"
#include "ntfsprogs/boot.h"
#include "ntfsprogs/attrdef.h"
/* #include "version.h" */
#include "logging.h"
#include "support.h"
#include "unistr.h"
#include "misc.h"
#include "compat.h" // for ffs()

#include "ntfs-3g_common.h"

extern struct ntfs_options opts;
extern ntfs_fuse_context_t *ctx;
...
static uint64_t crc64(uint64_t crc, const byte * data, size_t size)
	/*@*/
{
	static const uint64_t polynomial = 0x9a6c9329ac4bc9b5ULL;
	static const uint64_t xorout = 0xffffffffffffffffULL;
 ...
 * mkntfs_init_options
 */
static void mkntfs_init_options(void)
{
		...
		const u32 index_block_size)
{
	...
	if (index_block_size >= g_vol->cluster_size) {
		...
		if (index_block_size < (u32)opts2.sector_size) {
...
static BOOL mkntfs_open_partition(ntfs_volume *vol)
{
	 ...
	 * the volume.
	 */
	vol->dev = ntfs_device_alloc(opts2.dev_name, 0, &ntfs_device_default_io_ops, NULL);
	if (!vol->dev) {
	...
	if (!S_ISBLK(sbuf.st_mode)) {
		...
		if (!opts2.num_sectors) {
			if (!sbuf.st_size && !sbuf.st_blocks) {
				ntfs_log_error("You must specify the number of sectors.\n");
				goto done;
			}
			if (opts2.sector_size) {
				if (sbuf.st_size)
					opts2.num_sectors = sbuf.st_size / opts2.sector_size;
				else
					opts2.num_sectors = ((s64)sbuf.st_blocks << 9) / opts2.sector_size;
			} else {
				if (sbuf.st_size)
					opts2.num_sectors = sbuf.st_size / 512;
				else
					opts2.num_sectors = sbuf.st_blocks;
				opts2.sector_size = 512;
			}
		}
...
static BOOL mkntfs_override_vol_params(ntfs_volume *vol)
{
	...
	if (opts2.sector_size < 0) {
		opts2.sector_size = ntfs_device_sector_size_get(vol->dev);
		if (opts2.sector_size < 0) {
	...
	if ((opts2.sector_size - 1) & opts2.sector_size) {
		ntfs_log_error("The sector size is invalid.  It must be a "
			"power of two, e.g. 512, 1024.\n");
		return FALSE;
	}
	if (opts2.sector_size < 256 || opts2.sector_size > 4096) {
	...
	if (ntfs_device_block_size_set(vol->dev, opts2.sector_size))
	...
	if (opts2.num_sectors < 0) {
		opts2.num_sectors = ntfs_device_size_get(vol->dev,
				opts2.sector_size);
		if (opts2.num_sectors <= 0) {
	 ...
	 */
	i = 1;
	if (opts2.sector_size < 512)
		i = 512 / opts2.sector_size;
	opts2.num_sectors -= i;
	/* If user didn't specify the partition start sector, determine it. */
	if (opts2.part_start_sect < 0) {
		opts2.part_start_sect = ntfs_device_partition_start_sector_get(
				vol->dev);
		if (opts2.part_start_sect < 0) {
			ntfs_log_warning("The partition start sector was not "
				...
				vol->dev->d_name);
			opts2.part_start_sect = 0;
			winboot = FALSE;
		} else if (opts2.part_start_sect >> 32) {
	...
	if (opts2.sectors_per_track < 0) {
		opts2.sectors_per_track = ntfs_device_sectors_per_track_get(
				vol->dev);
		if (opts2.sectors_per_track < 0) {
			ntfs_log_warning("The number of sectors per track was "
				...
				"0.\n", vol->dev->d_name);
			opts2.sectors_per_track = 0;
			winboot = FALSE;
		} else if (opts2.sectors_per_track > 65535) {
			ntfs_log_warning("The number of sectors per track was "
	...
	if (opts2.heads < 0) {
		opts2.heads = ntfs_device_heads_get(vol->dev);
		if (opts2.heads < 0) {
	...
	volume_size = opts2.num_sectors * opts2.sector_size;
	...
	if (!vol->cluster_size) {
		...
		if (vol->cluster_size < (u32)opts2.sector_size)
			vol->cluster_size = opts2.sector_size;
		/*
	...
	if (vol->cluster_size > 128 * (u32)opts2.sector_size) {
	 ...
	 * with volume_size.
	 */
	if ((vol->nr_clusters != ((opts2.num_sectors * opts2.sector_size) /
			vol->cluster_size) ||
			(volume_size / opts2.sector_size) != opts2.num_sectors ||
	...
	if (vol->mft_record_size < (u32)opts2.sector_size)
		vol->mft_record_size = opts2.sector_size;
	...
	if (vol->indx_record_size < (u32)opts2.sector_size)
		vol->indx_record_size = opts2.sector_size;
...
static BOOL mkntfs_initialize_rl_mft(void)
{
	...
	switch (opts2.mft_zone_multiplier) {  /* % of volume size in clusters */
	case 4:
	...
	g_mftmirr_lcn = (opts2.num_sectors * opts2.sector_size >> 1)
			/ g_vol->cluster_size;
...
static BOOL mkntfs_fill_device_with_zeroes(void)
{
	...
	for (i = 0; (unsigned long)i < position; i++) {
		bw = mkntfs_write(g_vol->dev, g_buf, opts2.sector_size);
		if (bw != opts2.sector_size) {
			if (bw != -1 || errno != EIO) {
				ntfs_log_error("This should not happen.\n");
				return FALSE;
...
static int create_backup_boot_sector(u8 *buff)
{
	...
	/*
	 * Write the first max(512, opts2.sector_size) bytes from buf to the
	 * last sector, but limit that to 8192 bytes of written data since that
	 * is how big $Boot is (and how big our buffer is)..
	 */
	size = 512;
	if (size < opts2.sector_size)
		size = opts2.sector_size;
	if (g_vol->dev->d_ops->seek(g_vol->dev, (opts2.num_sectors + 1) *
			opts2.sector_size - size, SEEK_SET) == (fbx_off_t)-1) {
...
static BOOL mkntfs_create_root_structures(void)
{
	 ...
	 */
	bs->bpb.bytes_per_sector = cpu_to_le16(opts2.sector_size);
	bs->bpb.sectors_per_cluster = (u8)(g_vol->cluster_size /
			opts2.sector_size);
	bs->bpb.media_type = 0xf8; /* hard disk */
	bs->bpb.sectors_per_track = cpu_to_le16(opts2.sectors_per_track);
	ntfs_log_debug("sectors per track = %ld (0x%lx)\n",
			opts2.sectors_per_track, opts2.sectors_per_track);
	bs->bpb.heads = cpu_to_le16(opts2.heads);
	ntfs_log_debug("heads = %ld (0x%lx)\n", opts2.heads, opts2.heads);
	bs->bpb.hidden_sectors = cpu_to_le32(opts2.part_start_sect);
	...
	bs->number_of_sectors = cpu_to_sle64(opts2.num_sectors);
 ...
 * mkntfs_redirect
 */
static int mkntfs_redirect(void)
{
	u64 upcase_crc;
	int result = 1;
	...
	if (opts2.label) {
		g_vol->vol_name = strdup(opts2.label);
		if (!g_vol->vol_name) {
			ntfs_log_perror("Could not copy volume name");
			goto done;
		}
	}
	if (opts2.cluster_size >= 0)
		g_vol->cluster_size = opts2.cluster_size;
...
static void mkntfs_init_globals(void) {
	g_buf		  = NULL;
	...
	g_vol		  = NULL;
	g_mft_size		  = 0;
	g_mft_lcn		  = 0;		/* lcn of $MFT, $DATA attribute */
	g_mftmirr_lcn	  = 0;		/* lcn of $MFTMirr, $DATA */
	g_logfile_lcn	  = 0;		/* lcn of $LogFile, $DATA */
	g_logfile_size	  = 0;		/* in bytes, determined from volume_size */
	g_mft_zone_end	  = 0;		/* Determined from volume_size and mft_zone_multiplier, in clusters */
	g_num_bad_blocks	  = 0;		/* Number of bad clusters */
	g_bad_blocks		  = NULL;	/* Array of bad clusters */
	g_allocation	  = NULL;	/* Head of cluster allocations */
}

int ntfs_amifuse_format(const char *label, ULONG dostype)
{
	mkntfs_init_globals(); /* Clear globals */

	mkntfs_init_options(); /* Set up the options */

	opts2.dev_name = opts.device;
	opts2.quick_format = 1;
	if (label && label[0] != '\0') {
		opts2.label = label;
	}

	return mkntfs_redirect();
}
